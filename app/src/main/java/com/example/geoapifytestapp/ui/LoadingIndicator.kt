...
import com.example.geoapifytestapp.ui.theme.Teal200
...
fun InfiniteProgressView(
    ...
    Canvas(modifier.size(radius * 2)) {
        ...
        val _radius = (size.minDimension - higherStrokeWidth) / 2
        val halfSize = size / 2.0f
        val topLeft = Offset(
            halfSize.width - _radius,
            halfSize.height - _radius
        )
        val size = Size(_radius * 2, _radius * 2)
